...
namespace DistantLands.Lumen
{
    ...
    public class LumenEffectPlayer : MonoBehaviour
    {
        ...
        public void RedoEffect()
        {
            ...
            void DrawInstancedMesh(LumenEffectLayer layer, Vector3 posOffset, Vector3 rotOffset, Vector3 scaleOffset, Color colorOffset, float brightnessOffset)
            {
                ...
                if (layer.repeat && layer.repeatCount > 0)
                {
                    ...
                    for (int index = 0; index < adjustedCount; index++)
                    {
                        ...
                        {
                            DynamicRayLayer dynamicRay = (DynamicRayLayer)layer;
            ...
            void DrawLayer(LumenEffectLayer layer, Transform parent, Color layerFilter, float layerBrightness = 1)
            {
                if (!layer.active) return;
                ...
                if (layer is EffectStackLayer)
                {
                    ...
                    GameObject stackParent = new GameObject($"{effectStack.profile} Layer");
                    if (displayLayersInHierarchy)
                        stackParent.hideFlags = HideFlags.DontSaveInEditor;
                    else
                        stackParent.hideFlags = HideFlags.DontSaveInEditor | HideFlags.HideInHierarchy;
                    stackParent.transform.parent = parent;
                    stackParent.transform.localPosition = layer.position * scale;
                    stackParent.transform.localEulerAngles = layer.rotation;
                    stackParent.transform.localScale = layer.scale * scale;

                    foreach (LumenEffectLayer childLayer in effectStack.profile.layers)
                        DrawLayer(childLayer, stackParent.transform, layer.color, layer.brightness);

                    instantiatedLumenLayers.Add(new InstantiatedLumenLayer() { filter = null, renderer = null, transform = stackParent.transform });
                ...
                if (layer.repeat)
                {
                    GameObject obj = new GameObject($"{layer.mesh.name} Layer");
                    if (displayLayersInHierarchy)
                    ...
                    for (int index = 0; index < adjustedCount; index++)
                    {
                        ...
                        GameObject repeatedObj = new GameObject($"Repeated Layer {index}");
                        repeatedObj.transform.parent = obj.transform;
                        float completionPercentage = 0;
                        switch (layer.repeatDistribution)
                        {
                            case LumenEffectLayer.RepeatDistributionStyle.Uniform:
                                completionPercentage = (float)index / (float)Mathf.Max(adjustedCount - 1, 1);
                                repeatedObj.transform.localPosition = layer.position * scale + layer.repeatPositionSpread * completionPercentage;
                                repeatedObj.transform.localEulerAngles = layer.rotation + layer.repeatRotationSpread * completionPercentage;
                                repeatedObj.transform.localScale = scale * Vector3.Lerp(layer.scale, new Vector3(layer.scale.x * layer.repeatScaleSpread.x, layer.scale.y * layer.repeatScaleSpread.y, layer.scale.z * layer.repeatScaleSpread.z), completionPercentage);
                                break;
                            case LumenEffectLayer.RepeatDistributionStyle.Random:
                                completionPercentage = (float)random.NextDouble();
                                repeatedObj.transform.localPosition = layer.position * scale + layer.repeatPositionSpread * completionPercentage;
                        ...
                        if (layer.repeatVariation)
                        {
                            repeatedObj.transform.localPosition += new Vector3(RemappedRandom(layer.repeatPositionVariation.x), RemappedRandom(layer.repeatPositionVariation.y), RemappedRandom(layer.repeatPositionVariation.z));
                            repeatedObj.transform.localEulerAngles += new Vector3(RemappedRandom(layer.repeatRotationVariation.x), RemappedRandom(layer.repeatRotationVariation.y), RemappedRandom(layer.repeatRotationVariation.z));
                            repeatedObj.transform.localScale *= 1f - (float)random.NextDouble() * layer.repeatScaleVariation;
                        }

                        if (layer is DynamicRayLayer)
                        {
                            DynamicRayLayer dynamicRay = (DynamicRayLayer)layer;
                            SkinnedMeshRenderer renderer = repeatedObj.AddComponent<SkinnedMeshRenderer>();
                            renderer.sharedMaterial = layer.EffectMaterial;
                            renderer.sharedMesh = layer.mesh;
                            renderer.sharedMesh.bounds = new Bounds(Vector3.zero, new Vector3(dynamicRay.rayLength, dynamicRay.rayLength, dynamicRay.rayLength));
                            MaterialPropertyBlock propertyBlock = new MaterialPropertyBlock();
                            renderer.GetPropertyBlock(propertyBlock);
                            propertyBlock.SetColor(LumenShaderIDs.Color, color * controlledColor * layer.color * layer.repeatColors.Evaluate(completionPercentage) * layerFilter);
                            propertyBlock.SetFloat(LumenShaderIDs.Brightness, brightness * controlledBrightness * Mathf.Lerp(layer.brightness, layer.repeatBrightnessSpread, completionPercentage) * layerBrightness);
                            propertyBlock.SetFloat(LumenShaderIDs.AutoAssignSun, autoAssignSun ? 1f : 0f);
                            propertyBlock.SetFloat(LumenShaderIDs.UseLumenSunScript, useLumenSunScript ? 1f : 0f);
                            propertyBlock.SetVector(LumenShaderIDs.SunDirection, localSunDirection);

                            layer.DrawLayer(propertyBlock, renderer);
                            renderer.SetPropertyBlock(propertyBlock);
                        }
                        else
                        {
                            MeshRenderer renderer = repeatedObj.AddComponent<MeshRenderer>();
                            renderer.sharedMaterial = layer.EffectMaterial;
                            MeshFilter meshFilter = repeatedObj.AddComponent<MeshFilter>();
                            meshFilter.mesh = layer.mesh;
                            MaterialPropertyBlock propertyBlock = new MaterialPropertyBlock();
                            renderer.GetPropertyBlock(propertyBlock);
                            propertyBlock.SetColor(LumenShaderIDs.Color, color * controlledColor * layer.color * layer.repeatColors.Evaluate(completionPercentage) * layerFilter);
                ...
                else
                {
                    ...
                    //     obj.hideFlags = HideFlags.DontSaveInEditor | HideFlags.HideInHierarchy;
                    // obj.transform.parent = parent;
                    // obj.transform.localPosition = layer.position * scale;
                    // obj.transform.localEulerAngles = layer.rotation;
                    // obj.transform.localScale = layer.scale * scale;
                    // sharedBlock.Clear();
                    ...
                    // MaterialPropertyBlock propertyBlock = new MaterialPropertyBlock();
                    // renderer.GetPropertyBlock(propertyBlock);
                    // propertyBlock.SetColor(LumenShaderIDs.Color, color * controlledColor * layer.color * layerFilter);
                    // propertyBlock.SetFloat(LumenShaderIDs.Brightness, brightness * controlledBrightness * layer.brightness * layerBrightness);
                    // layer.DrawLayer(propertyBlock, renderer);
                    // renderer.SetPropertyBlock(propertyBlock);

                    // instantiatedLumenLayers.Add(new InstantiatedLumenLayer() { filter = null, renderer = renderer, transform = obj.transform });
